...
export default function GaleriAdminPage() {
  ...
  const { items, fetchGallery, deleteGalleryImage, toggleGalleryFeature, error: storeError } = useGalleryStore()
    ...
    <>
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="max-w-[1440px] mx-auto space-y-6">
          ...
          {/* Add Form Modal */}
          {showForm && <UploadPhotoForm onAdd={handleAdd} onCancel={() => setShowForm(false)} />}

          {/* Gallery Content */}
          {items.length === 0 ? (
            <EmptyState icon="photo_library" title="Belum Ada Foto Dokumentasi" description="Mulai unggah karya visual dan momen kegiatan studio MDPTV untuk ditampilkan pada website." actionText="Tambah Foto Baru" actionIcon="add_photo_alternate" onAction={() => setShowForm(true)} />
          ) : (
                ...
                return (
                  <div key={date} className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-primary font-display">{formatDate(date)}</h3>
                        <p className="text-[10px] text-on-surface-variant/50 font-medium">{groupItems.length} foto diunggah</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                      {groupItems.map((item, gi) => (
                        <div key={item.id || gi} className="relative group rounded-2xl sm:rounded-3xl overflow-hidden border border-outline-variant/15 hover:border-secondary/40 transition-all duration-300 shadow-xs bg-surface-container-lowest">
                          {item.image && <img src={getImageUrl(item.image)} alt={item.title || `Foto ${gi + 1}`} className="w-full aspect-square object-cover group-hover:scale-105 transition-transform duration-500" />}
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3.5">
                            <div className="flex items-center justify-end gap-1">
                              <button type="button" onClick={() => setDeleteTarget(item)} className="w-8 h-8 rounded-xl bg-rose-600/90 hover:bg-rose-600 text-white flex items-center justify-center transition-colors shadow-xs cursor-pointer" title="Hapus Foto">
                                <Icon name="delete" size="sm" className="!text-sm" />
                              </button>
                            </div>

                            <div>
                              {item.label && <span className="text-[9px] uppercase tracking-widest text-secondary-fixed-dim font-bold block mb-0.5">{item.label}</span>}
                              {item.title && <span className="text-xs font-bold text-white leading-tight block truncate">{item.title}</span>}
                            </div>
                          </div>
                        </div>
                      ))}
      ...
      <ConfirmModal isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} onConfirm={handleDeleteConfirm} title="Hapus Foto Dokumentasi" message={`Apakah Anda yakin ingin menghapus foto "${deleteTarget?.title || "ini"}"? Foto yang dihapus tidak dapat dipulihkan.`} confirmText="Hapus Foto" variant="danger" />
    </>
  )
}
