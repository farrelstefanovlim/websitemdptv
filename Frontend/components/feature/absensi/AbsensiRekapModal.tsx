...
export default function AbsensiRekapModal({ onClose }: AbsensiRekapModalProps) {
  ...
  const handleExport = () => {
    ...
    exportToExcel(recapData, cols, `rekap_absensi_${startDate}_to_${endDate}`, "Rekap");
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface-container-lowest rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col animate-in tracking-in fade-in zoom-in-95 duration-200 border-2 border-dashed border-secondary/30">
        ...
        <div className="p-5 sm:p-6 flex flex-col gap-4">
          <div>
            <label className="text-[10px] uppercase tracking-widest font-bold text-on-surface-variant/40 block mb-1.5">Mulai Tanggal</label>
            <DatePicker value={startDate} onChange={(e) => setStartDate(e.target.value)} />
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-widest font-bold text-on-surface-variant/40 block mb-1.5">Sampai Tanggal</label>
            <DatePicker value={endDate} onChange={(e) => setEndDate(e.target.value)} />
          </div>
        </div>

        <div className="p-5 sm:p-6 border-t border-outline-variant/10 flex gap-3 justify-end bg-surface-container-low/30 mt-auto">
          <Button variant="ghost" size="sm" onClick={onClose}>Batal</Button>
          <Button variant="primary" size="sm" onClick={handleExport} disabled={!startDate || !endDate}>
            <Icon name="download" size="sm" /> Export Excel
          </Button>
        </div>
      </div>
    </div>
