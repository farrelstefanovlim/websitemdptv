...
export default function WawancaraJawabanPage() {
  ...
  const handleSubmitInterview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`✅ Berhasil menyimpan jawaban wawancara untuk "${candidateName}"!`);
      setCandidateName("");
      setAnswers({});
      setInterviewNotes("");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/15 shadow-sm">
        <div>
          <h1 className="text-xl sm:text-2xl font-black tracking-tight text-primary font-display flex items-center gap-2">
            ...
            Pengisian Jawaban Wawancara ({selectedYear})
          </h1>
        ...
        <form onSubmit={handleSubmitInterview} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 rounded-2xl bg-surface-container-low/40 border border-outline-variant/15 text-xs">
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {interviewCandidates.length > 0 && (
                  <span className="text-[10px] font-bold text-secondary bg-secondary/10 px-2 py-0.5 rounded-md">
                    {interviewCandidates.length} Siap Interview
                  </span>
                )}
              </div>
                ...
                <div className="absolute left-0 right-0 top-full mt-1.5 bg-surface-container-lowest border border-outline-variant/20 rounded-2xl shadow-xl z-50 max-h-60 overflow-y-auto p-1.5 space-y-1">
                  ...
                  {filteredCandidates.length === 0 ? (
                    <div className="p-4 text-center text-xs text-on-surface-variant/60">
                      {interviewCandidates.length === 0 ? (
                        <span>Belum ada calon anggota berstatus &quot;Interview&quot; di Penerimaan.</span>
                      ) : (
                        <span>Tidak menemukan nama yang cocok. Anda dapat melanjutkan mengetik manual.</span>
                      )}
                    </div>
                  ) : (
                    filteredCandidates.map((c) => (
                      <div
                        ...
                        className="p-3 rounded-xl hover:bg-secondary/10 hover:text-secondary cursor-pointer transition-all flex items-center justify-between group"
                      >
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-primary group-hover:text-secondary truncate">
                            {c.name}
                          </p>
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 rounded-xl border border-outline-variant/20 bg-background text-primary focus:outline-none focus:border-secondary"
              />
            </div>
          </div>
                ...
                <div key={q.id} className="p-5 rounded-2xl border border-outline-variant/15 bg-background space-y-4 shadow-xs">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-on-surface-variant/60 uppercase font-semibold tracking-wider">
                        Tipe: {q.type === "MULTIPLE_CHOICE" ? "Pilihan Ganda" : "Essay / Isian Bebas"}
                      </span>
                    </div>
                  </div>
                  ...
                  {q.type === "MULTIPLE_CHOICE" && q.options ? (
                    ...
                    <div className="sm:ml-10">
                      <textarea
                        ...
                        className="w-full p-3.5 rounded-xl border border-outline-variant/20 bg-surface-container-lowest text-xs text-primary focus:outline-none focus:border-secondary"
                      />
                    </div>
                  )}
                </div>
          ...
          <div className="text-xs space-y-1.5">
            ...
            <textarea
              ...
              className="w-full p-3.5 rounded-xl border border-outline-variant/20 bg-background text-primary"
            />
          </div>
          ...
          <div className="pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-outline-variant/15">
            <p className="text-xs text-on-surface-variant/60">
              Form akan di-reset otomatis setelah simpan agar siap untuk wawancara peserta berikutnya.
            </p>
