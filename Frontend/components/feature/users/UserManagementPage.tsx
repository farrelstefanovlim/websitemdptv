...
export default function UserManagementPage() {
  ...
  const filtered = users.filter((u) => roleFilter === "all" || u.role === roleFilter).filter((u) => !search || u.fullName.toLowerCase().includes(search.toLowerCase()) || u.username.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()))
        ...
        <div className="max-w-[1440px] mx-auto space-y-6">
          ...
          <AdminPageHeader
            ...
            actions={
              <>
                ...
                <Button variant="primary" size="sm" onClick={() => setModal({ mode: "add", user: null })} startIcon={<Icon name="person_add" size="sm" />}>
                  Tambah Admin
                </Button>
              </>
            }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-4 sm:p-5 rounded-3xl border bg-surface-container-low border-outline-variant/15">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] sm:text-[11px] uppercase tracking-wider font-bold text-on-surface-variant/60">Total Akun</span>
              </div>
            ...
            {ALL_ROLES.map((r) => {
              ...
              return (
                <div key={r} className={`p-4 sm:p-5 rounded-3xl border transition-all ${isActive ? "bg-secondary/10 border-secondary/30 ring-1 ring-secondary/20" : "bg-surface-container-low border-outline-variant/15"}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`p-2 rounded-xl ${cfg.activeBg} ${cfg.color}`}>
                      <Icon name={cfg.icon} size="sm" filled />
                    </div>
                    <span className={`text-[10px] sm:text-[11px] uppercase tracking-wider font-bold ${cfg.color}`}>{cfg.label}</span>
                  </div>
                  <div className="text-2xl sm:text-3xl font-black text-primary font-display">{(counts as Record<string, number>)[r] || 0}</div>
                </div>
              )
            })}
          </div>
          ...
          <div className="mb-6 flex items-center gap-2 w-full">
            <div className="relative flex-1">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full h-full min-h-[46px] pl-11 pr-4 rounded-2xl border border-outline-variant/20 bg-surface-container-lowest text-sm text-primary shadow-sm focus:outline-none focus:border-secondary focus:ring-4 focus:ring-secondary/10 transition-all placeholder:text-on-surface-variant/40" placeholder="Cari nama, username, atau email..." />
            </div>
          ...
          <div className="flex flex-col gap-2.5">
            {filtered.length === 0 && (
              <div className="text-center py-16 bg-surface-container-lowest rounded-3xl border border-outline-variant/15">
                ...
                <p className="text-xs text-on-surface-variant/50 mt-1">Coba sesuaikan kata kunci pencarian atau role filter.</p>
              </div>
            )}
            ...
            {filtered.map((u) => {
              ...
              return (
                <div key={u.id} className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-outline-variant/15 bg-surface-container-lowest flex items-center justify-between gap-3 sm:gap-4 hover:border-outline-variant/30 transition-all">
                  {/* Avatar Icon */}
                  <div className="flex items-center gap-3.5 min-w-0 flex-1">
                    <div className={`w-10 h-10 sm:w-11 sm:h-11 rounded-2xl ${roleCfg.activeBg} border ${roleCfg.border} flex items-center justify-center shrink-0 relative font-bold text-sm font-display`}>
                      <Icon name={roleCfg.icon} filled className={`${roleCfg.color} !text-base sm:!text-lg`} />
                      {!u.isActive && <div className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-rose-500 border-2 border-white flex items-center justify-center" />}
                    </div>

                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-0.5">
                        <h3 className="text-sm sm:text-base font-bold text-primary font-display truncate">{u.fullName}</h3>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border ${roleCfg.activeBg} ${roleCfg.border} ${roleCfg.color}`}>{roleCfg.label}</span>
                        {!u.isActive && <span className="inline-flex px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-rose-50 text-rose-600 border border-rose-200">Nonaktif</span>}
                      </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button variant="icon" size="icon" onClick={() => toggleActive(u.id)} className={u.isActive ? "text-emerald-500 hover:bg-emerald-50" : "text-rose-400 hover:bg-rose-50"} title={u.isActive ? "Nonaktifkan Akun" : "Aktifkan Akun"}>
                      <Icon name={u.isActive ? "toggle_on" : "toggle_off"} size="sm" />
                    </Button>

                    <Button variant="icon" size="icon" onClick={() => setModal({ mode: "edit", user: u })} title="Edit User">
                      <Icon name="edit" size="sm" />
                    </Button>

                    {u.role !== "superadmin" && (
                      <Button variant="icon" size="icon" onClick={() => setDeleteTargetUser(u)} className="hover:bg-rose-50 hover:text-rose-600 text-on-surface-variant/40" title="Hapus Akun">
