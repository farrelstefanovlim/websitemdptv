    ...
    <div className="grid gap-3">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Button variant="none" size="none" onClick={addFeature} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-secondary/10 text-secondary text-[10px] font-bold uppercase tracking-wider hover:bg-secondary/20 transition-all">
            <Icon name="add" size="sm" className="!text-xs" /> Tambah Fitur
          </Button>
        </div>
        <div className="grid gap-3">
          {about.features.map((f, i) => (
            <div key={i} className="p-3 rounded-xl border border-outline-variant/15 bg-surface-container-low/50 grid gap-2">
              <div className="flex items-center justify-between">
                ...
                <Button variant="none" size="none" onClick={() => removeFeature(i)} className="text-error/60 hover:text-error transition-colors">
