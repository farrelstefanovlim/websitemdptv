...
export default function Footer() {
  ...
  return (
    <footer className="bg-white py-12 sm:py-16 md:py-20 border-t border-outline-variant/20 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 relative">
        {/* Top Section */}
        <AnimateOnScroll variant="fadeUp">
          <div className="flex flex-col lg:flex-row justify-between items-start gap-10 sm:gap-12 md:gap-16 mb-10 sm:mb-14 md:mb-16">
            ...
            <div className="max-w-md">
              <div className="flex items-center gap-3 sm:gap-4 mb-5 sm:mb-6">
                <div className="relative w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center shrink-0">
                  <Image src="/logo-mdptv.png" alt="MDPTV Logo" width={48} height={48} className="w-full h-full object-contain" />
                </div>
                <div className="text-3xl sm:text-4xl text-primary font-black tracking-tighter font-display">MDPTV</div>
              </div>
            ...
            {/* Link Columns */}
            <StaggerContainer className="grid grid-cols-2 sm:grid-cols-4 gap-8 sm:gap-10 md:gap-12 w-full lg:w-auto" staggerDelay={0.1}>
              {Object.entries(footerLinks).map(([title, links]) => (
                <StaggerItem key={title} className="flex flex-col gap-4 sm:gap-5">
                  ...
                  {links.map((link) => (
                    <Link key={link.label} href={link.href} className="text-on-surface-variant/60 hover:text-secondary transition-colors duration-300 text-sm font-medium">
                      {link.label}
                    </Link>
                  ))}
                </StaggerItem>
        ...
        <div className="pt-8 sm:pt-10 border-t border-outline-variant/15 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-8">
            <Link href="/privacy-policy" className="text-xs text-on-surface-variant/40 hover:text-primary transition-colors duration-300 uppercase tracking-widest">
              Privacy Policy
            </Link>
            <Link href="/terms-of-service" className="text-xs text-on-surface-variant/40 hover:text-primary transition-colors duration-300 uppercase tracking-widest">
              Terms of Service
            </Link>
          </div>
          <div className="flex gap-3">
            <Link href={f.instagram} className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl border border-outline-variant/20 flex items-center justify-center text-on-surface-variant/50 hover:bg-secondary hover:border-secondary hover:text-white transition-all duration-300">
              <Icon name="share" size="md" />
            </Link>
            <Link href={f.email} className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl border border-outline-variant/20 flex items-center justify-center text-on-surface-variant/50 hover:bg-secondary hover:border-secondary hover:text-white transition-all duration-300">
